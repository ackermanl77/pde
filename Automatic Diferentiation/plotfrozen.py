...
from pylab import plot, vlines

def PlotFrozenDist( dist, q=0.0001, N=200, linestyle='-', marker='o', color='k', **kwargs):
    ...
    if (continous):
        ...
        y = dist.pdf(x)
        plot(x, y, linestyle=linestyle, color=color, **kwargs)
    else:
        x = arange( dist.ppf(q0), dist.ppf(q1), dtype=int)
        y = dist.pmf(x)
        plot(x, y, marker, color=color, **kwargs)
        vlines(x, 0, y,  colors=color, linestyles=linestyle)
